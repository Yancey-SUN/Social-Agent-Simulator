...
type SharedRun = { id:string; createdAt:string|number; completedAt?:string|number|null; status:string; population:number; rounds:number; modelA:string; modelB:string; promptA:string; promptB:string; configJson:string; inputTokens:number; outputTokens:number; estimatedCostMicros:number };
...
const NAV=[['overview','◉','总览'],['run','✦','模拟运行'],['history','◷','历史实验'],['users','◎','用户宇宙'],['matches','⌁','匹配实验'],['logs','≡','事件日志']] as const;
...
export default function Home(){
  ...
  async function loadSharedRun(runId:string,goToOverview=true){
    setHistoryLoading(true);setRunError("");
    try{
      const response=await fetch(`/api/simulate?run_id=${encodeURIComponent(runId)}`);const saved=await response.json();if(!response.ok||!saved.run)throw new Error(saved.error||"实验结果不存在");
      const run=saved.run as SharedRun;let stored:any={};try{stored=JSON.parse(run.configJson||"{}")}catch{}
      const nextConfig:Config={...initialConfig,...stored,population:run.population,rounds:run.rounds,seed:String(stored.seed||`SHARED-${run.id.slice(-6)}`)};
      const nextUniverse=Math.max(run.population,Math.min(300,Number(stored.universeSize||run.population)));const names=Array.isArray(stored.customNames)?stored.customNames:[];
      const base=buildSimulation({...nextConfig,population:nextUniverse},names);
      const profileById=new Map((saved.profiles||[]).filter((x:any)=>x.variant==="A").map((x:any)=>[String(x.personaId),x]));
      const messagesById=new Map<string,any[]>();for(const row of (saved.messages||[]).filter((x:any)=>x.variant==="A").sort((a:any,b:any)=>a.turnIndex-b.turnIndex)){const key=String(row.personaId);messagesById.set(key,[...(messagesById.get(key)||[]),row])}
      base.users=base.users.map(persona=>{const row:any=profileById.get(String(persona.id));if(!row)return persona;let profile:any={};try{profile=JSON.parse(row.profileJson||"{}")}catch{}const transcript=(messagesById.get(String(persona.id))||[]).map((m:any)=>({from:(m.speaker==="user"?"user":"agent") as "user"|"agent",text:String(m.content)}));return {...persona,transcript,summary:String(profile.episode_summary||persona.summary),readiness:Number(profile.readiness||row.readiness||0),messages:transcript.length,memories:[...(profile.active_state||[]).slice(0,1).map((z:any)=>`最近：${z.claim}`),...(profile.stable_profile||[]).slice(0,1).map((z:any)=>`稳定倾向：${z.claim}`),`Social Intent：${profile.social_intent?.state||"unknown"}`],intent:(profile.social_intent?.state||"unknown") as Intent,status:Number(profile.readiness||row.readiness||0)>=nextConfig.threshold?"ready":"chatting"}});
      const byId=new Map(base.users.map(x=>[String(x.id),x]));
      const outcomeByMatch=new Map<string,any>();for(const out of (saved.outcomes||[]))outcomeByMatch.set(String(out.matchId),out);
      base.matches=(saved.matches||[]).map((row:any,i:number)=>{let research:any={};try{research=JSON.parse(row.researchJson||"{}")}catch{}const a=byId.get(String(row.personaAId))||base.users[0],b=byId.get(String(row.personaBId))||base.users[1]||base.users[0],out=outcomeByMatch.get(String(row.id));const aYes=Boolean(out?.acceptedA),bYes=Boolean(out?.acceptedB);return {id:i+1,remoteId:String(row.id),a,b,score:Number(row.score||0),relation:String(row.relationType||"unexpected"),resonance:String(research.resonance||""),value:String(research.mutual_value||""),status:aYes&&bYes?"group":aYes||bYes?"one-sided":"research",aYes,bYes,evidence:Array.isArray(research.evidence)?research.evidence:[]}});
      base.events=[{id:1,time:new Date(run.completedAt||run.createdAt).toLocaleTimeString("zh-CN",{hour12:false}),type:"RUN_REPLAYED",user:"SHARED DATABASE",message:`已加载 ${run.population} 人实验 · ${base.matches.length} 个关系机会`,tone:"green"}];
      setConfig(nextConfig);setUniverseSize(nextUniverse);setCustomNames(names);setData(base);setActiveRunId(run.id);setModelA(run.modelA);setModelB(run.modelB);setPromptA(run.promptA);setPromptB(run.promptB);setAbEnabled(stored.abEnabled!==false);setRealUsage({input:run.inputTokens||0,output:run.outputTokens||0,micros:run.estimatedCostMicros||0,calls:(saved.messages||[]).length+(saved.profiles||[]).length+(saved.matches||[]).length});setRealEvents(base.events);setSelectedUser(0);setSelectedMatch(0);setProgress(100);setSharedView(true);if(goToOverview)setTab("overview");
    }catch(e){setRunError(e instanceof Error?e.message:"共享实验加载失败")}finally{setHistoryLoading(false)}
  }
  async function refreshSharedRuns(autoLoad=false){try{const response=await fetch("/api/simulate");const saved=await response.json();setProviderReady(Boolean(saved.configured));const runs=(saved.runs||[]) as SharedRun[];setSharedRuns(runs);const latest=runs.find(x=>x.status==="completed");if(autoLoad&&latest)await loadSharedRun(latest.id,false)}catch{setProviderReady(false)}}
  useEffect(()=>{void refreshSharedRuns(true)},[]);
  ...
  const realRun=async()=>{
    ...
    try{
      const created=await api({action:"create_run",population:config.population,rounds:config.rounds,concurrency,modelA,modelB,promptA,promptB,config:{...config,universeSize,customNames,abEnabled}},controller.signal);setActiveRunId(created.runId);setSharedView(false);
      ...
      await api({action:"complete",experimentId:created.runId,status:"completed"},controller.signal);setProgress(100);pushRealEvent("RUN_COMPLETED","SYSTEM",`${jobs.length} 个 Agent 会话完成 · ${realMatches.length} 个机会`,"green");await refreshSharedRuns(false);
  ...
  return <main>
    ...
    <section className="canvas">
      <header><div><p className="eyebrow">SOCIAL AGENT SIMULATOR · {config.seed}</p><h1>{tab==="overview"?`${data.users.length} 个小精灵，正在真正对话。`:tab==="run"?"让一次关系实验可复现。":tab==="history"?"一次运行，所有人共享结果。":tab==="users"?"每个人，都在被慢慢理解。":tab==="matches"?"不是相似的人，而是值得认识的人。":"看见系统做出的每一个判断。"}</h1><p className="sub">{tab==="overview"?"DeepSeek Conversation → Memory → Profile → Matching → Outcome。":tab==="run"?"调整模型与 Prompt，重放真实对话和匹配链路。":tab==="history"?"共享数据库保存完整实验；查看回放不会调用模型。":tab==="users"?"检查真实对话、Memory、Intent 与画像证据。":tab==="matches"?"研究关系假设、双边价值和长期结果。":"全链路 trace：可筛选、可解释、可导出。"}</p></div><div className="header-actions"><button className="ghost" onClick={()=>fileRef.current?.click()}>↑ 导入人设</button><input ref={fileRef} hidden type="file" accept=".json,.csv" onChange={importPersonas}/><button className="run" onClick={()=>setTab("run")} disabled={running}>{running?`运行中 ${progress}%`:"▶ 新建实验"}</button></div></header>
      {tab==="overview"&&<>
        {sharedView&&<div className="shared-banner"><span><b>共享实验回放</b><small>其他人打开网址即可看到这次结果；回放不会再次调用模型。</small></span><button onClick={()=>setTab("history")}>查看历史实验 →</button></div>}
      ...
      {tab==="history"&&<article className="card history"><div className="history-head"><div><h2>共享实验历史</h2><p>所有完成的真实实验都保存在服务器；任何访问者都能只读回放，不消耗 API。</p></div><button className="ghost" onClick={()=>refreshSharedRuns(false)} disabled={historyLoading}>{historyLoading?"刷新中…":"↻ 刷新"}</button></div>{runError&&<div className="run-error">{runError}</div>}<div className="history-grid">{sharedRuns.length?sharedRuns.map(run=><button key={run.id} className={`history-card ${activeRunId===run.id?"selected":""}`} onClick={()=>loadSharedRun(run.id)} disabled={historyLoading}><span className={`run-state ${run.status}`}>{run.status==="completed"?"已完成":run.status==="running"?"运行中":"已中止"}</span><time>{new Date(run.createdAt).toLocaleString("zh-CN",{hour12:false})}</time><h3>{run.population} 人 · {run.rounds} 轮{run.configJson.includes('"abEnabled":true')?" · A/B":""}</h3><p>{run.modelA} / {run.promptA}</p><div><span>Input <b>{(run.inputTokens||0).toLocaleString()}</b></span><span>Output <b>{(run.outputTokens||0).toLocaleString()}</b></span><span>Cost <b>${((run.estimatedCostMicros||0)/1e6).toFixed(4)}</b></span></div><em>{activeRunId===run.id?"正在查看":"打开完整结果 →"}</em></button>):<div className="empty-history"><b>还没有共享实验</b><span>完成第一轮真实实验后，它会自动出现在这里。</span></div>}</div><div className="history-note"><b>隐私边界</b><span>实验对话、画像和匹配结果会共享；DeepSeek Key 永远不会写入实验记录。</span></div></article>}
