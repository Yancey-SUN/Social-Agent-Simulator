...
export default function Home(){
  ...
  const dryRun=()=>{setRunning(true);setProgress(0);let p=0;const t=setInterval(()=>{p+=3+Math.random()*6;if(p>=100){clearInterval(t);setData(buildSimulation(config,customNames));setProgress(100);setRunning(false)}else setProgress(Math.round(p))},90)};
  ...
  const api=async(payload:Record<string,unknown>,signal?:AbortSignal)=>{const res=await fetch("/api/simulate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload),signal});const out=await res.json();if(!res.ok)throw new Error(out.error||`API ${res.status}`);return out};
  ...
  const realRun=async()=>{
    if(!providerReady){setRunError("请先在站点环境变量中配置 DEEPSEEK_API_KEY");return}
    ...
    try{
      ...
      const seeds=buildSimulation(config,customNames).users.slice(0,config.population).map((p,i)=>({...p,baziPrior:BAZI_PRIORS[i%BAZI_PRIORS.length]}));
  ...
  return <main>
    ...
    <section className="canvas">
      ...
      {tab==="run"&&<div className="run-layout"><article className="card controls"><CardHead title="实验设置" sub="真实调用、全量留痕、可中止" badge="DEEPSEEK"/><div className={`provider ${providerReady?"connected":"missing"}`}><i/><span><b>{providerReady?"DeepSeek 已连接":"需要配置 API Key"}</b><small>{providerReady?"服务端密钥已就绪":"Site settings → DEEPSEEK_API_KEY"}</small></span></div><label className="field"><span>实验种子 <small>相同人设可复现</small></span><input value={config.seed} onChange={e=>update("seed",e.target.value)}/></label><Slider label="模拟用户" value={config.population} min={2} max={100} step={2} suffix=" 人" onChange={v=>update("population",v)}/><Slider label="每人对话轮次" value={config.rounds} min={1} max={8} step={1} suffix=" 轮" onChange={v=>update("rounds",v)}/><Slider label="并发数" value={concurrency} min={1} max={6} step={1} suffix="" onChange={setConcurrency}/><Slider label="Readiness 阈值" value={config.threshold} min={.3} max={.9} step={.01} format={pct} onChange={v=>update("threshold",v)}/><div className="ab-switch"><span><b>A/B 对照实验</b><small>相同人设分别运行两个版本</small></span><button className={abEnabled?"on":""} onClick={()=>setAbEnabled(!abEnabled)}><i/></button></div><Variant label="A" model={modelA} prompt={promptA} setModel={setModelA} setPrompt={setPromptA}/>{abEnabled&&<Variant label="B" model={modelB} prompt={promptB} setModel={setModelB} setPrompt={setPromptB}/>}<div className="mode"><span>运行方式</span><button className={runMode==="real"?"selected":""} onClick={()=>setRunMode("real")}>真实 LLM</button><button className={runMode==="synthetic"?"selected":""} onClick={()=>setRunMode("synthetic")}>零成本 dry run</button></div>{runError&&<div className="run-error">{runError}</div>}<button className="start" disabled={running||(runMode==="real"&&!providerReady)} onClick={runMode==="real"?realRun:dryRun}>{running?`运行中 · ${progress}%`:`▶ 运行 ${config.population} 人${abEnabled&&runMode==="real"?" × A/B":""}`}</button>{running&&<button className="stop" onClick={()=>abortRef.current?.abort()}>停止实验</button>}<p className="budget-note">预算保护：默认 20 人、并发 3；100 人可后续直接调高。</p></article>
